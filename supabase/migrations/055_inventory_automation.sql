...
-- ============================================
-- AUTOMATED INVENTORY SYNC FUNCTIONS
-- ============================================

-- Function to sync inventory on order confirmation
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
    ...
    LOOP
      -- Check if variant_id exists in the item
      IF v_item.value->>'variant_id' IS NOT NULL THEN
        -- Get current available inventory
        SELECT available_quantity INTO v_available
        FROM inventory
        WHERE variant_id = (v_item.value->>'variant_id')::uuid
        FOR UPDATE;
        ...
        SET 
          ...
          updated_at = NOW()
        WHERE variant_id = (v_item.value->>'variant_id')::uuid
          AND available_quantity >= (v_item.value->>'quantity')::integer;
        ...
        -- Check for low stock
        PERFORM check_low_stock((v_item.value->>'variant_id')::uuid);
      END IF;
    END LOOP;
  END IF;
...
BEGIN
  ...
  LOOP
    -- Restore the reserved quantity to available
    UPDATE inventory
    SET 
    ...
    INSERT INTO inventory_movements (
      ...
      'reservation',
      v_reservation.id,
      'Stock reservation expired and released'
    );
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(DISTINCT i.variant_id)::INTEGER as total_variants,
    COUNT(CASE WHEN i.available_quantity <= COALESCE(it.low_stock_threshold, 10) 
               AND i.available_quantity > 0 THEN 1 END)::INTEGER as low_stock_items,
    COUNT(CASE WHEN i.available_quantity = 0 THEN 1 END)::INTEGER as out_of_stock_items,
    SUM(i.available_quantity * pv.price)::DECIMAL as total_stock_value,
    COUNT(CASE WHEN i.reserved_quantity > 0 THEN 1 END)::INTEGER as items_on_order
  FROM inventory i
  JOIN product_variants pv ON pv.id = i.variant_id
  LEFT JOIN inventory_thresholds it ON it.variant_id = i.variant_id;
END;
...
-- ============================================
-- SCHEDULED JOBS (Run these via cron or Supabase Edge Functions)
-- ============================================

-- Job to check all products for low stock (run daily)
...
BEGIN
  ...
  LOOP
    PERFORM check_low_stock(v_variant.variant_id);
    v_count := v_count + 1;
  END LOOP;
...
-- Job to clean up expired reservations (run every hour)
CREATE OR REPLACE FUNCTION scheduled_cleanup_reservations()
RETURNS INTEGER AS $$
BEGIN
  RETURN cleanup_expired_stock_reservations();
END;
$$ LANGUAGE plpgsql;
