...
-- Check how many blazers exist before insert
SELECT COUNT(*) as existing_blazers FROM products_enhanced WHERE category = 'Blazers';
...
categories.forEach(category => {
  ...
  '${productFolder.includes('black') ? 'Black' :
     ...
     productFolder.includes('purple') ? 'Purple' :
     productFolder.includes('teal') ? 'Teal' :
     productFolder.includes('white') && !productFolder.includes('off-white') ? 'White' :
     productFolder.includes('off-white') ? 'Off White' :
     'Multi-Color'}',
  '{"primary": "${category === 'velvet' ? 'Velvet' : category === 'sparkle' ? 'Sparkle Fabric' : 'Polyester Blend'}", "composition": {"${category === 'velvet' ? 'Cotton Velvet' : 'Polyester'}": ${category === 'velvet' ? 85 : 75}, "${category === 'velvet' ? 'Silk' : 'Viscose'}": ${category === 'velvet' ? 15 : 25}}}'::jsonb,
  'Slim Fit',
  '{
    "hero": {
...
sql += `-- SUMMARY\n`;
sql += `-- ${'='.repeat(44)}\n`;
sql += `-- Total products in SQL: ${productCount}\n`;
sql += `-- Categories: ${categories.join(', ')}\n\n`;

sql += `-- Verify insert results
SELECT 
  subcategory,
  COUNT(*) as count
...
-- Count total blazers after insert
SELECT 
  'Blazers After Insert' as status,
...
// Write SQL file
const outputPath = '/Users/ibrahim/Desktop/Super-Admin/INSERT_ALL_BLAZERS_FIXED.sql';
