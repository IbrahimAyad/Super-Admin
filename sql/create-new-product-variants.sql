...
-- Rules:
-- 1. Suspender & Bowtie Sets: One Size only
-- 2. Vest & Tie Sets: XS through 6XL (10 sizes)
-- 3. Suits & Tuxedos: Standard sizes 36R-54R, 36L-54L (NO SHORT sizes)
-- 4. Double-Breasted Suits: Same as suits
-- 5. Stretch Suits: Same as suits
-- 6. Dress Shirts: Neck sizes 14.5-18

-- First ensure products table is synced from products_enhanced
...
BEGIN
  ...
  LOOP
    ...
    ELSIF product_record.sku LIKE 'ACC-VTS-%' OR 
          ...
          LOWER(product_record.name) LIKE '%vest%sparkle%' THEN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '=========================================';
  RAISE NOTICE 'NEW COLLECTION VARIANTS COMPLETE!';
...
-- Verify the results
SELECT 'Summary by Category' as report;
SELECT 
  p.category,
...
SELECT 'Sample Products and Sizes' as report;
SELECT 
  p.name,
  p.category,
  p.sku,
