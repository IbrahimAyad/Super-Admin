-- Fix ALL Database Errors - Final Version
-- Run this in Supabase SQL Editor to fix all 400/404 errors

-- 1. Fix get_recent_orders (drop all versions first)
...
CREATE TABLE metrics (
  ...
  metric_name TEXT NOT NULL UNIQUE,  -- Add UNIQUE constraint for ON CONFLICT
...
-- 5. Fix transfer_guest_cart function (drop existing first)
DROP FUNCTION IF EXISTS transfer_guest_cart(text, uuid);
DROP FUNCTION IF EXISTS transfer_guest_cart(varchar, uuid);
...
-- 6. Fix get_sync_progress_by_category function
DROP FUNCTION IF EXISTS get_sync_progress_by_category(text);
...
-- 9. Create comprehensive policies (drop existing first)
-- Metrics policies
DROP POLICY IF EXISTS "Users can read metrics" ON metrics;
DROP POLICY IF EXISTS "Users can manage metrics" ON metrics;

CREATE POLICY "Users can manage metrics" ON metrics
...
DROP POLICY IF EXISTS "Users can manage logs" ON logs;

CREATE POLICY "Users can manage logs" ON logs
  FOR ALL TO authenticated
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "Users can manage custom_orders" ON custom_orders;

CREATE POLICY "Users can manage custom_orders" ON custom_orders
  FOR ALL TO authenticated
  USING (true)
  WITH CHECK (true);
...
GRANT EXECUTE ON FUNCTION get_recent_orders(int) TO authenticated;
GRANT EXECUTE ON FUNCTION transfer_guest_cart(text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION get_sync_progress_by_category(text) TO authenticated;

-- 11. Insert default data to prevent empty table errors
-- First check if user is authenticated
DO $$
BEGIN
  -- Add current user as admin if authenticated
  IF auth.uid() IS NOT NULL THEN
    INSERT INTO admin_users (user_id, role, permissions)
    VALUES (auth.uid(), 'super_admin', '["all"]'::jsonb)
    ON CONFLICT (user_id) DO NOTHING;
  END IF;
END $$;

-- Add some default metrics (with proper ON CONFLICT)
...
-- 13. Optimize existing queries with ANALYZE
ANALYZE orders;
ANALYZE products;
ANALYZE customers;
ANALYZE metrics;
