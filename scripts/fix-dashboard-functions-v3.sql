-- =====================================================
-- FIX DASHBOARD RPC FUNCTIONS AND PERMISSIONS (V3)
-- Fixed version - no stock_quantity column assumption
-- =====================================================

-- 1. Drop and recreate inventory table
...
BEGIN
  ...
  WHERE COALESCE(i.available_quantity, 100) < threshold
     OR i.id IS NULL
...
-- 13. Populate inventory for existing products with default values
-- Since we don't have stock_quantity column, we'll use default of 100
...
SELECT 
  ...
  100 as available_quantity,  -- Default available
...
-- 14. Add some test vendors if needed
INSERT INTO vendors (name, email, dropship_enabled)
VALUES 
  ('Default Vendor', 'vendor@example.com', false)
ON CONFLICT DO NOTHING;

-- 15. Test the functions
SELECT 'Testing get_dashboard_stats:' as test;
SELECT get_dashboard_stats();

SELECT 'Testing get_recent_orders:' as test;
